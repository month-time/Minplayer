$(function () {
//    if (location.pathname == "/player_test/") {
        created_paging();
        analysis_playUrl();
//    } else {
//        created_flashplayer();
...
function analysis_playUrl() {
    ...
    script.onload = function () {
        ...
        var voide = [];
        for (var i = 0; i < 2; i++) {voide.push(document.querySelectorAll("#player_code li")[i].innerText);}
        voide[0] = voide[0].match(/type=.*?\|/g);
        for (var i = 0; i < voide[0].length; i++) {voide[0][i] = voide[0][i].replace(/\|/, "");}
