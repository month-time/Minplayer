(function (e) {
	...
	return function d(c, e, t) {
		...
		for (var n = "function" == typeof require && require, s = 0; s < t.length; s++) r(t[s]);
		return r
	}({
		1: [function (e, t) {
			function n(e, t) {
			...
			o(n, s.Readable), n.prototype._read = function () {
				...
				if (e._reading && 0 !== e._missing) {
					...
					if (!e._notifying) {
						if (e._notifying = !0, e._torrent.destroyed) return e._destroy(new Error("Torrent removed"));
						var t = e._piece;
		...
		2: [function (e, t) {
			(function (n) {
				...
				d(r, s), Object.defineProperty(r.prototype, "downloaded", {
					get: function () {
						if (!this._torrent.bitfield) return 0;
						for (var e = 0, t = this._startPiece; t <= this._endPiece; ++t)
							if (this._torrent.bitfield.get(t)) e += t === this._endPiece ? this._torrent.lastPieceLength : this._torrent.pieceLength;
							else {
			...
			"render-media": 87,
			"stream-to-blob": 110,
			"stream-to-blob-url": 109,
			"stream-with-known-length-to-buffer": 111
		}],
		3: [function (e, t, n) {
			function r(e, t) {
				var n = this;
		...
		5: [function (e, t) {
			(function (r, o) {
				...
				S(i, _), Object.defineProperty(i.prototype, "timeRemaining", {
						...
						return function (t) {
							return e.destroyed ? t(new Error("torrent is destroyed")) : void e.store.get(n, function (o, s) {
								return e.destroyed ? t(new Error("torrent is destroyed")) : o ? r.nextTick(t, null) : void O(s, function (r) {
									...
									if (r === e._hashes[n]) {
										if (!e.pieces[n]) return;
										e._debug("piece verified %s", n), e._markVerified(n)
									} else e._debug("piece invalid %s", n);
									t(null)
								})
							})
						}
					...
					var r;
					return r = "string" == typeof e ? F.createTCPOutgoingPeer(e, t) : F.createWebRTCPeer(e, t), t._peers[r.id] = r, t._peersLength += 1, "string" == typeof e && (t._queue.push(r), t._drain()), r
					...
					var t = F.createWebSeedPeer(e, this);
						...
						if (!e.requests.length)
							for (var n = d._selections.length; n--;) {
								...
								if ("rarest" === d.strategy)
									for (var s = r.from + r.offset, i = r.to, a = {}, c = 0, p = t(s, i, a); c < i - s + 1 && (o = d._rarityMap.getRarestPiece(p), !(0 > o));) {
					...
					function o() {
						...
						var r = n(1, e.requests.length) * U.BLOCK_LENGTH / t,
							...
							s = 0;
						return function (e) {
							if (!o || d.bitfield.get(e)) return !0;
							for (var n = d.pieces[e].missing; s < d.wires.length; s++) {
								var i = d.wires[s],
									a = i.downloadSpeed();
								if (!(a < V) && !(a <= t) && i.peerPieces.get(e) && !(0 < (n -= a * r))) return o--, !1
							}
							return !0
						}
					}
					...
					function i(n) {
						...
						for (var r = o(), a = 0; a < d._selections.length; a++) {
							var i = d._selections[a],
								c;
							if ("rarest" === d.strategy)
								for (var l = i.from + i.offset, u = i.to, f = {}, h = 0, m = t(l, u, f, r); h < u - l + 1 && (c = d._rarityMap.getRarestPiece(m), !(0 > c));) {
									...
									for (c = i.from + i.offset; c <= i.to; c++)
										if (e.peerPieces.get(c) && r(c)) {
											for (; d._request(e, c, d._critical[c] || n););
											if (!(e.requests.length < p)) return i.priority && s(a), !0
										}
						}
						return !1
					}
					...
					var u = l ? d(c(e, G, s.pieceLength), s.maxWebConns) : a(e, G);
					if (p >= u) return !1;
					var f = s.pieces[t],
						h = l ? f.reserveRemaining() : f.reserve();
					if (-1 === h && n && s._hotswap(e, t) && (h = l ? f.reserveRemaining() : f.reserve()), -1 === h) return !1;
					var m = s._reservations[t];
					m || (m = s._reservations[t] = []);
					var g = m.indexOf(null); - 1 === g && (g = m.length), m[g] = e;
					var i = f.chunkOffset(h),
						_ = l ? f.chunkLengthRemaining(h) : f.chunkLength(h);
					return e.request(t, i, _, function n(r, d) {
						if (!s.destroyed) {
							if (!s.ready) return s.once("ready", function () {
								n(r, d)
							});
							if (m[g] === e && (m[g] = null), f !== s.pieces[t]) return o();
							if (r) return s._debug("error getting piece %s (offset: %s length: %s) from %s: %s", t, i, _, e.remoteAddress + ":" + e.remotePort, r.message), l ? f.cancelRemaining(h) : f.cancel(h), void o();
							if (s._debug("got piece %s (offset: %s length: %s) from %s", t, i, _, e.remoteAddress + ":" + e.remotePort), !f.set(h, d, e)) return o();
							var a = f.flush();
							O(a, function (e) {
								if (!s.destroyed) {
									if (e === s._hashes[t]) {
									...
									} else s.pieces[t] = new U(f.length), s.emit("warning", new Error("Piece " + t + " failed verification"));
									o()
								}
							})
						}
					...
					if ("function" != typeof W) throw new Error("node.js-only method");
					if (this.destroyed) throw new Error("torrent is destroyed");
					var t = new W(this, e);
			...
			"addr-to-ip-port": 7,
			bitfield: 13,
			"chunk-store-stream/write": 29,
			debug: 33,
			events: 37,
			fs: 26,
			"fs-chunk-store": 53,
			"immediate-chunk-store": 43,
			inherits: 44,
			multistream: 61,
			net: 25,
			os: 25,
			...
			"random-iterate": 75,
			"run-parallel": 92,
			"run-parallel-limit": 91,
			"simple-get": 96,
			"simple-sha1": 100,
			speedometer: 104,
			"torrent-discovery": 116,
			"torrent-piece": 119,
			uniq: 122,
			ut_metadata: 126,
			ut_pex: 25,
		...
		6: [function (e, t) {
			...
			c(r, l), r.prototype._init = function () {
				var e = this;
				e.setKeepAlive(!0), e.once("handshake", function (t) {
					if (!e.destroyed) {
						e.handshake(t, e.webPeerId);
						for (var n = e._torrent.pieces.length, r = new o(n), s = 0; s <= n; s++) r.set(s, !0);
						e.bitfield(r)
					}
				}), e.once("interested", function () {
				...
				if (1 >= h.length) m = [{
					...
					if (1 > g.length) return o(new Error("Could not find file corresponnding to web seed range request"));
					m = g.map(function (e) {
						var t = e.offset + e.length - 1,
							r = c.url + ("/" === c.url[c.url.length - 1] ? "" : "/") + e.path;
						return {
							url: r,
							fileOffsetInRange: n(e.offset - l, 0),
							start: n(l - e.offset, 0),
							end: d(t, f - e.offset)
						}
					})
				}
				...
				1 < m.length && (b = s.alloc(r)), m.forEach(function (n) {
					...
					var d = n.url,
						c = n.start,
						p = n.end;
					i("Requesting url=%s pieceIndex=%d offset=%d length=%d start=%d end=%d", d, e, t, r, c, p);
					var l = {
						url: d,
						method: "GET",
						headers: {
		...
		7: [function (e, t) {
			...
			t.exports = function (e) {
				if (1e5 == o && t.exports.reset(), !r[e]) {
					var s = n.exec(e);
					if (!s) throw new Error("invalid addr: " + e);
					r[e] = [s[1], +s[2]], o += 1
				}
				return r[e]
		...
		8: [function (e, t, n) {
			...
			n.byteLength = function (e) {
				...
				var p = 0;
				for (n = 0; n < o; n += 4) s = a[e.charCodeAt(n)] << 18 | a[e.charCodeAt(n + 1)] << 12 | a[e.charCodeAt(n + 2)] << 6 | a[e.charCodeAt(n + 3)], d[p++] = 255 & s >> 16, d[p++] = 255 & s >> 8, d[p++] = 255 & s;
		...
		9: [function (t, n) {
			(function (t) {
				...
				o.bytes = 0, o.position = 0, o.data = null, o.encoding = null, o.next = function () {
					...
					o.position++;
					for (var e = {}; o.data[o.position] !== s;) e[o.buffer()] = o.next();
					return o.position++, e
				}, o.list = function () {
					o.position++;
					for (var e = []; o.data[o.position] !== s;) e.push(o.next());
					return o.position++, e
				}, o.integer = function () {
					var e = o.find(s),
		...
		10: [function (e, t) {
			...
			n.buffer = function (e, t) {
				...
				e.push(s);
				for (var r = 0, i = Object.keys(t).sort(), d = i.length, a; r < d; r++) a = i[r], null == t[a] || (n.buffer(e, a), n._encode(e, t[a]));
				e.push(o)
			}, n.list = function (e, t) {
				var r = 0,
					s = t.length;
				for (e.push(d); r < s; r++) null != t[r] && n._encode(e, t[r]);
				e.push(o)
			}, t.exports = n
		}, {
			"safe-buffer": 94
		...
		12: [function (e, t) {
			t.exports = function (e, t, n, r, o) {
				...
				for (; r <= o;)
					if (s = r + (o - r >> 1), i = +n(e[s], t, s, e), 0 > i) r = s + 1;
					else if (0 < i) o = s - 1;
				else return s;
				return ~r
			}
		...
		14: [function (e, t) {
			...
			l(r, h.Duplex), r.prototype.setKeepAlive = function (e) {
				...
				for (var s in this._ext) this._ext[s].onHandshake(r, o, n);
		...
		16: [function (e, t, n) {
			...
			n = t.exports = s.debug = s["default"] = s, n.coerce = function (e) {
				...
				for (var t = ("string" == typeof e ? e : "").split(/[\s,]+/), r = t.length, o = 0; o < r; o++) t[o] && (e = t[o].replace(/\*/g, ".*?"), "-" === e[0] ? n.skips.push(new RegExp("^" + e.substr(1) + "$")) : n.names.push(new RegExp("^" + e + "$")))
		...
		17: [function (e, t) {
			(function (n) {
				function r(e) {
					...
					if (i.call(d), e || (e = {}), !e.peerId) throw new Error("Option `peerId` is required");
				...
				a(r, i), r.scrape = function (e, t) {
					...
					return s.on("scrape", function (e) {
						...
						return o.from(e, "hex")
					}) : o.from(e.infoHash, "hex"), s.scrape({
						infoHash: e.infoHash
					}), s
			...
			"./lib/client/http-tracker": 25,
			"./lib/client/udp-tracker": 25,
		...
		19: [function (e, n) {
			...
			c(o, h), o.prototype.DEFAULT_ANNOUNCE_INTERVAL = 30000, o.prototype.announce = function (e) {
				...
				var n = this;
				if (e || (e = s), n.destroyed) return e(null);
				...
				if (delete m[n.announceUrl], i.on("error", s), i.once("close", e), !n.expectingResponse) return t();
		...
		27: [function (n, r, o) {
			...
			function L(e, t, n) {
				...
				for (var r = [], o = t; o < n;) {
					...
					if (o + a <= n) {
						var c, p, l, u;
						1 == a ? 128 > s && (i = s) : 2 == a ? (c = e[o + 1], 128 == (192 & c) && (u = (31 & s) << 6 | 63 & c, 127 < u && (i = u))) : 3 == a ? (c = e[o + 1], p = e[o + 2], 128 == (192 & c) && 128 == (192 & p) && (u = (15 & s) << 12 | (63 & c) << 6 | 63 & p, 2047 < u && (55296 > u || 57343 < u) && (i = u))) : 4 == a ? (c = e[o + 1], p = e[o + 2], l = e[o + 3], 128 == (192 & c) && 128 == (192 & p) && 128 == (192 & l) && (u = (15 & s) << 18 | (63 & c) << 12 | (63 & p) << 6 | 63 & l, 65535 < u && 1114112 > u && (i = u))) : void 0
					}
			...
			function z(e, t) {
				for (var n = [], r = 0, o, s, i; r < e.length && !(0 > (t -= 2)); ++r) o = e.charCodeAt(r), s = o >> 8, i = o % 256, n.push(i), n.push(s);
				return n
			}
			...
			o.kMaxLength = Q, c.TYPED_ARRAY_SUPPORT = function () {
				...
				if (e === t) return 0;
				for (var n = e.length, r = t.length, o = 0, s = d(n, r); o < s; ++o)
					if (e[o] !== t[o]) {
						n = e[o], r = t[o];
						break
					}
			...
			c.prototype.slice = function (e, t) {
				...
				for (var o = this[e], s = 1, d = 0; ++d < n && (s *= 256);) o += this[e + d] * s;
				return s *= 128, o >= s && (o -= t(2, 8 * n)), o
			}, c.prototype.readIntBE = function (e, n, r) {
				e >>>= 0, n >>>= 0, r || O(e, n, this.length);
				for (var o = n, s = 1, i = this[e + --o]; 0 < o && (s *= 256);) i += this[e + --o] * s;
				...
				var d = 1,
					a = 0;
				for (this[n] = 255 & e; ++a < r && (d *= 256);) this[n + a] = 255 & e / d;
				return n + r
				...
				var d = r - 1,
					i = 1;
				for (this[n + d] = 255 & e; 0 <= --d && (i *= 256);) this[n + d] = 255 & e / i;
				return n + r
				...
				var d = 0,
					i = 1,
					a = 0;
				for (this[n] = 255 & e; ++d < r && (i *= 256);) 0 > e && 0 == a && 0 !== this[n + d - 1] && (a = 1), this[n + d] = 255 & (e / i >> 0) - a;
				return n + r
				...
				var d = r - 1,
					i = 1,
					a = 0;
				for (this[n + d] = 255 & e; 0 <= --d && (i *= 256);) 0 > e && 0 == a && 0 !== this[n + d + 1] && (a = 1), this[n + d] = 255 & (e / i >> 0) - a;
				return n + r
		...
		29: [function (e, t) {
			function n(e, t, o) {
				var i = this;
				if (!(i instanceof n)) return new n(e, t, o);
				if (s.Writable.call(i, o), o || (o = {}), !e || !e.put || !e.get) throw new Error("First argument must be an abstract-chunk-store compliant store");
				if (t = +t, !t) throw new Error("Second argument must be a chunk length");
				i._blockstream = new r(t, {
					zeroPadding: !1
				}), i._blockstream.on("data", function (t) {
		...
		30: [function (e, t) {
			t.exports = function (e, t, n) {
				for (var r = Infinity, s = 0, d = t.length - 1, a, i, c; s <= d && (a = s + (d - s >> 1), c = t[a] - e, 0 > c ? s = a + 1 : 0 < c ? d = a - 1 : void 0, c = o(c), c < r && (r = c, i = a), t[a] !== e););
				return n ? i : t[i]
			}
		}, {}],
		31: [function (e, t, n) {
					...
					function s() {
						O(e.map(function (e) {
							return function (t) {
								...
								else if (o.isBuffer(e)) n.getStream = w(e), n.length = e.length;
		...
		34: [function (e, t, n) {
			...
			n = t.exports = s.debug = s["default"] = s, n.coerce = function (e) {
				...
				var t = ("string" == typeof e ? e : "").split(/[\s,]+/),
					r = t.length,
					o;
				for (o = 0; o < r; o++) t[o] && (e = t[o].replace(/\*/g, ".*?"), "-" === e[0] ? n.skips.push(new RegExp("^" + e.substr(1) + "$")) : n.names.push(new RegExp("^" + e + "$")));
		...
		36: [function (e, t) {
			var n = e("once"),
				...
				i = function (e, t, d) {
					...
					return o(e) ? (e.on("complete", f), e.on("abort", g), e.req ? _() : e.on("request", _)) : l && !a && (e.on("end", u), e.on("close", u)), s(e) && e.on("exit", m), e.on("end", h), e.on("finish", f), !1 !== t.error && e.on("error", d), e.on("close", g),
						function () {
		...
		42: [function (e, n, s) {
			s.read = function (n, r, o, a, c) {
				var p = 8 * c - a - 1,
					...
					d = n[r + h],
					s, e;
				for (h += i, s = d & (1 << -f) - 1, d >>= -f, f += p; 0 < f; s = 256 * s + n[r + h], h += i, f -= 8);
				...
				var h = 8 * f - u - 1,
					...
					d = 0 > a || 0 === a && 0 > 1 / a ? 1 : 0,
					s, w, m;
				for (a = o(a), isNaN(a) || a === Infinity ? (w = isNaN(a) ? 1 : 0, s = g) : (s = r(Math.log(a) / Math.LN2), 1 > a * (m = t(2, -s)) && (s--, m *= 2), a += 1 <= s + _ ? y / m : y * t(2, 1 - _), 2 <= a * m && (s++, m /= 2), s + _ >= g ? (w = 0, s = g) : 1 <= s + _ ? (w = (a * m - 1) * t(2, u), s += _) : (w = a * t(2, _ - 1) * t(2, u), s = 0)); 8 <= u; n[p + b] = 255 & w, b += i, w /= 256, u -= 8);
				for (s = s << u | w, h += u; 0 < h; n[p + b] = 255 & s, b += i, s /= 256, h -= 8);
				n[p + b - i] |= 128 * d
			}
		}, {}],
		43: [function (e, t) {
		...
		53: [function (e, t) {
			(function (e) {
				...
				t.exports = n, n.prototype.put = function (e, t, n) {
					...
					var s = t.offset || 0,
						i = t.length || o.length - s;
					r(n, null, o.slice(s, i + s))
		...
		54: [function (e, t, n) {
			(function (t) {
				...
				function c(e, t, n) {
					...
					for (var r = 0; r < e.length; r++) a(e[r], t, n + 4 * r)
				}
				...
				["mvhd", "tkhd", "mdhd", "vmhd", "smhd", "stsd", "esds", "stsz", "stco", "stss", "stts", "ctts", "stsc", "dref", "elst", "hdlr", "mehd", "trex", "mfhd", "tfhd", "tfdt", "trun"].forEach(function (e) {
					...
					e = e.slice(t);
					for (var r = n - t, o = d(e.readUInt8(42), 31), s = {
					...
					e = e.slice(t);
					for (var n = e.readUInt32BE(0), r = Array(n), o = 0, s; o < n; o++) s = 8 * o + 4, r[o] = {
					...
					e = e.slice(t);
					for (var n = e.readUInt32BE(0), r = Array(n), o = 0, s; o < n; o++) s = 12 * o + 4, r[o] = {
					...
					for (var d = 0, i; d < s.length; d++) i = 12 * d + 4, r.writeUInt32BE(s[d].trackDuration || 0, i), r.writeUInt32BE(s[d].mediaTime || 0, i + 4), a(s[d].mediaRate || 0, r, i + 8);
					...
					return r.fill(0, 0, s), r.write(e.handlerType || "", 4, 4, "ascii"), p(e.name || "", r, 20), n.hdlr.encode.bytes = s, r
		...
		55: [function (e, t, n) {
			(function (e) {
				...
				n.Descriptor = {}, n.Descriptor.decode = function (r, o, s) {
					...
					var p = t[i],
						l;
					return l = n[p] ? n[p].decode(r, d, s) : {
						buffer: new e(r.slice(d, d + a))
					}, l.tag = i, l.tagName = p, l.length = d - o + a, l.contentsLen = a, l
		...
		56: [function (e, t, n) {
			(function (t) {
				...
				i.encode = function (e, n, r) {
					...
					if (d[a]) {
						c.otherBoxes = [];
						for (var p = d[a], l = r, u; 8 <= s - l;)
							if (u = i.decode(n, l, s), l += u.length, 0 <= p.indexOf(u.type)) c[u.type] = u;
						...
						var m = o[a].decode;
						c = m(n, r, s)
					} else c.buffer = new t(n.slice(r, s));
		...
		58: [function (e, t) {
			(function (n, r) {
				...
				t.exports = s, a(s, d.Readable), s.prototype.mediaData = s.prototype.mdat = function (e, t) {
					...
					var s;
					if (e.encodeBufferLen && (s = new r(e.encodeBufferLen)), e.stream) e.buffer = null, s = c.encode(e, s), this.push(s), this._stream = e.stream, this._stream.on("readable", this._onreadable), this._stream.on("end", this._onend), this._stream.on("end", t), this._forward();
					else {
						s = c.encode(e, s);
						var i = this.push(s);
		...
		60: [function (e, t) {
			function n(e) {
				if (e += "", !(100 < e.length)) {
					...
					if (t) {
						...
						return "years" === n || "year" === n || "yrs" === n || "yr" === n || "y" === n ? r * d : "days" === n || "day" === n || "d" === n ? r * u : "hours" === n || "hour" === n || "hrs" === n || "hr" === n || "h" === n ? r * l : "minutes" === n || "minute" === n || "mins" === n || "min" === n || "m" === n ? r * p : "seconds" === n || "second" === n || "secs" === n || "sec" === n || "s" === n ? r * c : "milliseconds" === n || "millisecond" === n || "msecs" === n || "msec" === n || "ms" === n ? r : void 0
					}
				}
			}
			...
			function o(e) {
				var t = Math.round;
				return e >= u ? t(e / u) + "d" : e >= l ? t(e / l) + "h" : e >= p ? t(e / p) + "m" : e >= c ? t(e / c) + "s" : e + "ms"
			}
		...
		64: [function (e, t) {
			(function (n) {
				function r(e) {
					...
					return t.pieceLength = e.info["piece length"], t.lastPieceLength = (l.offset + l.length) % t.pieceLength || t.pieceLength, t.pieces = s(e.info.pieces), t
				}
		...
		67: [function (e, n) {
			for (var r = e("closest-to"), o = 1024, s = 13, i = []; 22 > s++;) i.push(t(2, s));
		...
		71: [function (t, n, o) {
			(function (t) {
				(function (s) {
					...
					function p(e) {
						for (var t = [], n = 0, r = e.length, o, s; n < r;) o = e.charCodeAt(n++), 55296 <= o && 56319 >= o && n < r ? (s = e.charCodeAt(n++), 56320 == (64512 & s) ? t.push(((1023 & o) << 10) + (1023 & s) + 65536) : (t.push(o), n--)) : t.push(o);
						return t
					}
					...
					function h(e, t, n) {
						var r = 0;
						for (e = n ? P(e / E) : e >> 1, e += P(e / t); e > R * C >> 1; r += v) e = P(e / R);
					...
					function m(e) {
						...
						for (f = 0 < c ? c + 1 : 0; f < o;) {
							for (m = s, g = 1, _ = v;; _ += v) {
								if (f >= o && d("invalid-input"), y = u(e.charCodeAt(f++)), (y >= v || y > P((x - s) / g)) && d("overflow"), s += y * g, b = _ <= n ? S : _ >= n + C ? C : _ - n, y < b) break;
					...
					function g(e) {
						var r = [],
							o, n, s, i, a, c, l, u, m, g, t, _, y, b, w;
						for (e = p(e), _ = e.length, o = I, n = 0, a = B, c = 0; c < _; ++c) t = e[c], 128 > t && r.push(O(t));
						for (s = i = r.length, i && r.push(L); s < _;) {
							...
							for (y = s + 1, l - o > P((x - n) / y) && d("overflow"), n += (l - o) * y, o = l, c = 0; c < _; ++c)
								if (t = e[c], t < o && ++n > x && d("overflow"), t == o) {
									for (u = n, m = v;; m += v) {
										if (g = m <= a ? S : m >= a + C ? C : m - a, u < g) break;
		...
		77: [function (e, t) {
			...
			t.exports = r, o(r, s.Writable), r.prototype._write = function (e, t, r) {
				for (var o = this, s = !0;;) {
					...
					if (0 === o._queue.length) return o._buffer = e, void(o._cb = r);
					o._buffer = null;
					var i = o._queue[0],
						d = n(i.start - o._position, 0),
						a = i.end - o._position;
					if (d >= e.length) return o._position += e.length, r(null);
					var c;
					if (a > e.length) {
		...
		78: [function (e, t) {
			...
			d.inherits(n, a);
			for (var p = i(c.prototype), l = 0, u; l < p.length; l++) u = p[l], n.prototype[u] || (n.prototype[u] = c.prototype[u]);
		...
		80: [function (e, t) {
			(function (n, o) {
				...
				function a(t, n) {
					O = O || e("./_stream_duplex"), t = t || {}, this.objectMode = !!t.objectMode, n instanceof O && (this.objectMode = this.objectMode || !!t.readableObjectMode);
					var o = t.highWaterMark,
						s = this.objectMode ? 16 : 16384;
					this.highWaterMark = o || 0 === o ? o : s, this.highWaterMark = r(this.highWaterMark), this.buffer = new z, this.length = 0, this.pipes = null, this.pipesCount = 0, this.flowing = null, this.ended = !1, this.endEmitted = !1, this.reading = !1, this.sync = !0, this.needReadable = !1, this.emittedReadable = !1, this.readableListening = !1, this.resumeScheduled = !1, this.destroyed = !1, this.defaultEncoding = t.defaultEncoding || "utf8", this.awaitDrain = 0, this.readingMore = !1, this.decoder = null, this.encoding = null, t.encoding && (!G && (G = e("string_decoder/").StringDecoder), this.decoder = new G(t.encoding), this.encoding = t.encoding)
				}
				...
				Object.defineProperty(c.prototype, "destroyed", {
					...
					var n = this._readableState,
						r;
					return n.objectMode ? r = !0 : "string" == typeof e && (t = t || n.defaultEncoding, t !== n.encoding && (e = j.from(e, t), t = ""), r = !0), p(this, e, t, !1, r)
				...
				c.prototype.read = function (e) {
					...
					var _ = !1,
						y = !1;
					return u.on("data", i), d(e, "error", a), e.once("close", c), e.once("finish", p), e.emit("pipe", u), f.flowing || (W("pipe resume"), u.resume()), e
					...
					for (var i = 0; i < K.length; i++) e.on(K[i], o.emit.bind(o, K[i]));
			...
			"./_stream_duplex": 78,
			"./internal/streams/BufferList": 83,
			"./internal/streams/destroy": 84,
			"./internal/streams/stream": 85,
			_process: 69,
			"core-util-is": 31,
			events: 37,
			inherits: 44,
		...
		81: [function (e, t) {
			...
			function o(e) {
				...
				i.call(this, e), this._transformState = new n(this);
				var t = this;
			...
			var i = e("./_stream_duplex"),
				d = e("core-util-is");
			d.inherits = e("inherits"), d.inherits(o, i), o.prototype.push = function (e, t) {
		...
		82: [function (e, t) {
			(function (n, o) {
				...
				function c(t, n) {
					T = T || e("./_stream_duplex"), t = t || {}, this.objectMode = !!t.objectMode, n instanceof T && (this.objectMode = this.objectMode || !!t.writableObjectMode);
					var o = t.highWaterMark,
						i = this.objectMode ? 16 : 16384;
					this.highWaterMark = o || 0 === o ? o : i, this.highWaterMark = r(this.highWaterMark), this.finalCalled = !1, this.needDrain = !1, this.ending = !1, this.ended = !1, this.finished = !1, this.destroyed = !1;
				...
				function k(e, t) {
					...
					if (e._writev && n && n.next) {
						...
						i.entry = n;
						for (var d = 0, a = !0; n;) o[d] = n, n.isBuf || (a = !1), n = n.next, d += 1;
						...
						for (; n;) {
							...
							if (m(e, t, !1, u, c, p, l), n = n.next, t.writing) break
						}
				...
				A.inherits(p, R), c.prototype.getBuffer = function () {
				...
				var M;
				"function" == typeof Symbol && Symbol.hasInstance && "function" == typeof Function.prototype[Symbol.hasInstance] ? (M = Function.prototype[Symbol.hasInstance], Object.defineProperty(p, Symbol.hasInstance, {
					value: function (e) {
					...
					var r = this._writableState,
						o = !1,
						s = d(e) && !r.objectMode;
					return s && !P.isBuffer(e) && (e = i(e)), "function" == typeof t && (n = t, t = null), s ? t = "buffer" : !t && (t = r.defaultEncoding), "function" != typeof n && (n = a), r.ended ? l(this, n) : (s || u(this, r, e, n)) && (r.pendingcb++, o = h(this, r, s, e, t, n)), o
		...
		84: [function (e, t) {
			...
			t.exports = {
				destroy: function (e, t) {
					...
					return s || i ? void(t ? t(e) : e && (!this._writableState || !this._writableState.errorEmitted) && r(n, this, e)) : void(this._readableState && (this._readableState.destroyed = !0), this._writableState && (this._writableState.destroyed = !0), this._destroy(e || null, function (e) {
		...
		87: [function (e, t, n) {
			...
			n.render = function (e, t, n, o) {
				...
				if ("function" == typeof n && (o = n, n = {}), n || (n = {}), o || (o = function () {}), s(e), d(n), "string" == typeof t && (t = document.querySelector(t)), t && ("VIDEO" === t.nodeName || "AUDIO" === t.nodeName)) throw new Error("Invalid video/audio node argument. Argument must be root element that video/audio tag will be appended to.");
				r(e, function (e) {
				...
				(function () {
					function n(e) {
						...
						for (var t = {
								...
								for (var s = (t >> 2) + 1; s < e.length; s++) e[s] = 0
							}, c = function (e, t, n) {
								...
								switch (i) {
									...
									case 1:
										e[0 | o + 1 - (i << 1)] = s.charCodeAt(n + 2);
									case 2:
										e[0 | o + 2 - (i << 1)] = s.charCodeAt(n + 1);
									case 3:
										e[0 | o + 3 - (i << 1)] = s.charCodeAt(n);
								}
								if (!(r < d + i)) {
									for (c = 4 - i; c < a; c = 0 | c + 4) t[o + c >> 2] = s.charCodeAt(n + c) << 24 | s.charCodeAt(n + c + 1) << 16 | s.charCodeAt(n + c + 2) << 8 | s.charCodeAt(n + c + 3);
									switch (d) {
										case 3:
											e[0 | o + a + 1] = s.charCodeAt(n + a + 2);
										case 2:
											e[0 | o + a + 2] = s.charCodeAt(n + a + 1);
										case 1:
								...
								if (!(r < d + i)) {
									for (c = 4 - i; c < a; c = 0 | c + 4) t[0 | o + c >> 2] = s[n + c] << 24 | s[n + c + 1] << 16 | s[n + c + 2] << 8 | s[n + c + 3];
									switch (d) {
										case 3:
											e[0 | o + a + 1] = s[n + a + 2];
										case 2:
								...
								if (!(r < a + d)) {
									for (l = 4 - d; l < c; l = 0 | l + 4) t[0 | s + l >> 2] = p[l] << 24 | p[l + 1] << 16 | p[l + 2] << 8 | p[l + 3];
									switch (a) {
										case 3:
											e[0 | s + c + 1] = p[c + 2];
										case 2:
						...
						var _ = function (e, n) {
							...
							w = function (e, n, r, o, s) {
								var i = r;
								b(e, n, r), s && (i = y(r, o)), t.core.hash(i, t.padMaxChunkLen)
							},
							k = function (e, t) {
						...
						this.digest = this.digestFromString = this.digestFromBuffer = this.digestFromArrayBuffer = function (e) {
							...
							var n = 0,
								...
								s;
							for (t.offset += r; n < r;) s = d(r - n, t.maxChunkLen - o), b(e, n, s, o), o += s, n += s, o === t.maxChunkLen && (t.core.hash(t.maxChunkLen, t.padMaxChunkLen), o = 0);
							return this
							...
							return {
								hash: function (e, t) {
									...
									var n = 0,
										...
										m = 0,
										g = 0;
									for (s = 0 | r[t + 320 >> 2], d = 0 | r[t + 324 >> 2], c = 0 | r[t + 328 >> 2], l = 0 | r[t + 332 >> 2], f = 0 | r[t + 336 >> 2], n = 0;
										(0 | n) < (0 | e); n = 0 | n + 64) {
										for (i = s, a = d, p = c, u = l, h = f, o = 0; 64 > (0 | o); o = 0 | o + 4) g = 0 | r[n + o >> 2], m = 0 | (0 | (s << 5 | s >>> 27) + (d & c | ~d & l)) + (0 | (0 | g + f) + 1518500249), f = l, l = c, c = d << 30 | d >>> 2, d = s, s = m, r[e + o >> 2] = g;
										for (o = 0 | e + 64;
											(0 | o) < (0 | e + 80); o = 0 | o + 4) g = (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) << 1 | (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) >>> 31, m = 0 | (0 | (s << 5 | s >>> 27) + (d & c | ~d & l)) + (0 | (0 | g + f) + 1518500249), f = l, l = c, c = d << 30 | d >>> 2, d = s, s = m, r[o >> 2] = g;
										for (o = 0 | e + 80;
											(0 | o) < (0 | e + 160); o = 0 | o + 4) g = (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) << 1 | (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) >>> 31, m = 0 | (0 | (s << 5 | s >>> 27) + (d ^ c ^ l)) + (0 | (0 | g + f) + 1859775393), f = l, l = c, c = d << 30 | d >>> 2, d = s, s = m, r[o >> 2] = g;
										for (o = 0 | e + 160;
											(0 | o) < (0 | e + 240); o = 0 | o + 4) g = (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) << 1 | (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) >>> 31, m = 0 | (0 | (s << 5 | s >>> 27) + (d & c | d & l | c & l)) + (0 | (0 | g + f) - 1894007588), f = l, l = c, c = d << 30 | d >>> 2, d = s, s = m, r[o >> 2] = g;
										for (o = 0 | e + 240;
											(0 | o) < (0 | e + 320); o = 0 | o + 4) g = (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) << 1 | (r[o - 12 >> 2] ^ r[o - 32 >> 2] ^ r[o - 56 >> 2] ^ r[o - 64 >> 2]) >>> 31, m = 0 | (0 | (s << 5 | s >>> 27) + (d ^ c ^ l)) + (0 | (0 | g + f) - 899497514), f = l, l = c, c = d << 30 | d >>> 2, d = s, s = m, r[o >> 2] = g;
			...
			(function (n) {
				function r(e) {
					...
					if (t._id = a(4).toString("hex").slice(0, 7), t._debug("new peer %o", e), e = Object.assign({
							...
							return e
						}, t.stream = e.stream || !1, t.trickle = void 0 === e.trickle || e.trickle, t._earlyMessage = null, t.destroyed = !1, t.connected = !1, t.remoteAddress = void 0, t.remoteFamily = void 0, t.remotePort = void 0, t.localAddress = void 0, t.localPort = void 0, t._wrtc = e.wrtc && "object" == typeof e.wrtc ? e.wrtc : i(), !t._wrtc)
		...
		101: [function (e, t) {
			(function (n) {
				...
				var o = e("safe-buffer").Buffer,
					...
					l = 65536;
				i(r, a.Duplex), r.WEBSOCKET_SUPPORT = !!p, r.prototype.send = function (e) {
					this._ws.send(e)
				}, r.prototype.destroy = function (e) {
					this._destroy(null, e)
				}, r.prototype._destroy = function (e, t) {
		...
		104: [function (e, t) {
			...
			s.unref && s.unref(), t.exports = function (e) {
				...
				return function (e) {
					var a = n - d & r;
					for (a > t && (a = t), d = n; a--;) i == t && (i = 0), s[i] = s[0 == i ? t - 1 : i - 1], i++;
					e && (s[i - 1] += e);
					var c = s[i - 1],
						p = s.length < t ? 0 : s[i == t ? 0 : i];
					return s.length < o ? c : (c - p) * o / s.length
				}
			}
		}, {}],
		105: [function (e, t, n) {
			(function (t) {
				...
				d.request = function (e, n) {
					...
					var s = -1 === t.location.protocol.search(/^https?:$/) ? "http:" : "",
						d = e.protocol || s,
						a = e.hostname || e.host,
						c = e.port,
						p = e.path || "/";
				...
				a(h, p.Writable), h.prototype.setHeader = function (e, t) {
					...
					if (!e._destroyed) {
						var t = e._opts,
							s = e._headers,
							i = null;
						"GET" !== t.method && "HEAD" !== t.method && (d.blobConstructor ? i = new r.Blob(e._body.map(function (e) {
							return l(e)
						}), {
							type: (s["content-type"] || {}).value || ""
						}) : i = o.concat(e._body).toString());
						var a = [];
		...
		114: [function (e, t, n) {
			(function (e) {
				...
				n.encode = function (n) {
					...
					for (var r = 0, o = 0, s = 0, i = 0, d = new e(8 * t(n)); r < n.length;) {
						var a = n[r];
					...
					for (var c = new e(s(5 * t.length / 8)), p = 0; p < t.length && !(61 === t[p]); p++) {
						var i = t[p] - 48;
						if (i < o.length) r = o[i], 3 >= n ? (n = (n + 5) % 8, 0 == n ? (a |= r, c[d] = a, d++, a = 0) : a |= 255 & r << 8 - n) : (n = (n + 5) % 8, a |= 255 & r >>> n, c[d] = a, d++, a = 255 & r << 8 - n);
		...
		119: [function (e, t) {
			(function (e) {
				...
				n.BLOCK_LENGTH = r, n.prototype.chunkLength = function (e) {
					...
					if (!this.init()) return !1;
					for (var o = t.length, i = s(o / r), d = 0; d < i; d++)
						if (!this._buffer[e + d]) {
							var a = d * r,
								c = t.slice(a, a + r);
		...
		124: [function (e, t, n) {
			...
			r.prototype.parse = function (e, t, n) {
				...
				if (!_[C] && (B || C && !y[C])) {
					for (var I = -1, L = 0, i; L < f.length; L++) i = v.indexOf(f[L]), -1 !== i && (-1 == I || i < I) && (I = i);
					var T, A;
					A = -1 === I ? v.lastIndexOf("@") : v.lastIndexOf("@", I), -1 !== A && (T = v.slice(0, A), v = v.slice(A + 1), this.auth = decodeURIComponent(T)), I = -1;
					for (var L = 0, i; L < l.length; L++) i = v.indexOf(l[L]), -1 !== i && (-1 === I || i < I) && (I = i); - 1 === I && (I = v.length), this.host = v.slice(0, I), v = v.slice(I), this.parseHost(), this.hostname = this.hostname || "";
					...
					var F = this.port ? ":" + this.port : "",
						p = this.hostname || "";
					this.host = p + F, this.href += this.host, U && (this.hostname = this.hostname.substr(1, this.hostname.length - 2), "/" !== v[0] && (v = "/" + v))
				}
				if (!g[E])
					for (var L = 0, P = u.length, D; L < P; L++)
				...
				for (var B = S.slice(-1)[0], I = (n.host || e.host || 1 < S.length) && ("." === B || ".." === B) || "" === B, L = 0, T = S.length; 0 <= T; T--) B = S[T], "." === B ? S.splice(T, 1) : ".." === B ? (S.splice(T, 1), L++) : L && (S.splice(T, 1), L--);
				if (!x && !v)
					for (; L--; L) S.unshift("..");
			...
			t.exports = function (e) {
				function t(t) {
					d.call(this), this._wire = t, this._metadataComplete = !1, this._metadataSize = null, this._remainingRejects = null, this._fetching = !1, this._bitfield = new r(0, {
						grow: p
					}), o.isBuffer(e) && this.setMetadata(e)
				}
		...
		130: [function (e, t) {
			(function (n) {
				...
				o.prototype._generateFragment = function (e, t) {
					...
					for (var s = r.samples[o].dts, i = 0, d = [], a = o, c; a < r.samples.length && (c = r.samples[a], !(c.sync && c.dts - s >= r.timeScale * 1)); a++) {
						i += c.size;
						var p = d.length - 1;
					...
					for (var r = this, o = r._tracks[e], s = [], i = t, d; i < n; i++) d = o.samples[i], s.push({
			...
			(function (n, r) {
				function o(e) {
					...
					return s instanceof o ? void(u.call(s), !e && (e = {}), s.peerId = "string" == typeof e.peerId ? e.peerId : d.isBuffer(e.peerId) ? e.peerId.toString("hex") : d.from(B + w(9).toString("base64")).toString("hex"), s.peerIdBuffer = d.from(s.peerId, "hex"), s.nodeId = "string" == typeof e.nodeId ? e.nodeId : d.isBuffer(e.nodeId) ? e.nodeId.toString("hex") : w(20).toString("hex"), s.nodeIdBuffer = d.from(s.nodeId, "hex"), s._debugId = s.peerId.toString("hex").substring(0, 7), s.destroyed = !1, s.listening = !1, s.torrentPort = e.torrentPort || 0, s.dhtPort = e.dhtPort || 0, s.tracker = e.tracker === void 0 ? {} : e.tracker, s.torrents = [], s.maxConns = +e.maxConns || 55, s._debug("new webtorrent (peerId %s, nodeId %s, port %s)", s.peerId, s.nodeId, s.torrentPort), s.tracker && ("object" != typeof s.tracker && (s.tracker = {}), e.rtcConfig && (console.warn("WebTorrent: opts.rtcConfig is deprecated. Use opts.tracker.rtcConfig instead"), s.tracker.rtcConfig = e.rtcConfig), e.wrtc && (console.warn("WebTorrent: opts.wrtc is deprecated. Use opts.tracker.wrtc instead"), s.tracker.wrtc = e.wrtc), r.WRTC && !s.tracker.wrtc && (s.tracker.wrtc = r.WRTC)), "function" == typeof v ? s._tcpPool = new v(s) : n.nextTick(function () {
						s._onListening()
					}), s._downloadSpeed = k(), s._uploadSpeed = k(), !1 !== e.dht && "function" == typeof l ? (s.dht = new l(f({
						nodeId: s.nodeId
						...
						var e = s.dht.address();
						e && (s.dhtPort = e.port)
					}), s.dht.setMaxListeners(0), s.dht.listen(s.dhtPort)) : s.dht = !1, s.enableWebSeeds = !1 !== e.webSeeds, "function" == typeof m && null != e.blocklist ? m(e.blocklist, {
						headers: {
				...
				h(o, u), o.WEBRTC_SUPPORT = b.WEBRTC_SUPPORT, Object.defineProperty(o.prototype, "downloadSpeed", {
					...
					function r() {
						if (!d.destroyed)
							for (var e = 0, n = d.torrents.length, r; e < n; e++)
					...
					var d = this;
					if (d.destroyed) throw new Error("client is destroyed");
					if ("function" == typeof t) return d.add(e, null, t);
